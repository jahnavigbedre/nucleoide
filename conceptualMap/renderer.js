...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Terminal types
    const terminalTypes = ['powershell', 'bash', 'nodejs'];
    const terminals = {};
    const fitAddons = {};
    const terminalContainers = {};

    // Create containers for each terminal inside the visible terminal tab panel
    const terminalPanelContent = document.querySelector('.terminal-tab-panel[data-tab="terminal"] .terminal-content');
    // Remove placeholder text if present
    if (terminalPanelContent) terminalPanelContent.innerHTML = '';
    terminalTypes.forEach(type => {
        const container = document.createElement('div');
        container.id = `terminal-container-${type}`;
        container.style.width = '100%';
        container.style.height = '100%';
        container.style.display = 'none';
        terminalPanelContent.appendChild(container);
        terminalContainers[type] = container;
    });
    ...
    terminalTypes.forEach(type => {
        ...
        term.open(terminalContainers[type]);
        fitAddon.fit();
        terminals[type] = term;
        fitAddons[type] = fitAddon;

        // Send input to main process
        term.onData(data => {
            ipcRenderer.send('terminal-input', { type, data });
        });

        // Listen for output from main process
        ipcRenderer.on(`terminal-output-${type}`, (event, data) => {
            term.write(data);
        });
    });
    ...
    function showTerminal(type) {
        terminalTypes.forEach(t => {
            terminalContainers[t].style.display = (t === type) ? 'block' : 'none';
            if (t === type) {
                fitAddons[t].fit();
            }
        });
    ...
    // Listen for terminal item clicks in the UI
    setTimeout(() => {
        ...
        if (terminalsList) {
            terminalsList.addEventListener('click', (e) => {
                ...
                const type = terminalItem.getAttribute('data-terminal');
                showTerminal(type);
            });
            // Show the first terminal by default
            showTerminal('powershell');
        }
    }, 100);

    // Fit terminals on window resize
    window.addEventListener('resize', () => {
        terminalTypes.forEach(type => fitAddons[type].fit());
    });

    // Initialize all terminals in the main process
    ipcRenderer.send('init-terminals');

    // ...existing code for tab switching, file explorer, etc...
});